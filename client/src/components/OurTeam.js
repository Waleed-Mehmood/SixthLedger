...
const OurTeam = () => {
  return (
    <div className="bg-[#202427]">
      ...
      >
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-20">
          Meet Our <span className="text-[#f94713]">Team</span>
        </h2>
        <div className="flex flex-col md:flex-row justify-center items-center gap-12">
          {teamMembers.map((member, index) => (
            <div key={index} className="flex flex-col items-center">
              <div className="relative bg-gradient-to-b from-[#F94714] to-[#401205] pt-6 pb-0 rounded-lg overflow-hidden">
                <img
                  src={member.image}
                  alt={member.name}
              ...
              <h3 className="text-white text-base sm:text-lg md:text-xl font-semibold mt-4">
                {member.name}
              </h3>
              <p className="text-white text-xs sm:text-sm md:text-base bg-[#F94714] px-2 py-1 rounded-md mt-2">
                {member.title}
              </p>
            </div>
          ))}
